...
export function transition(previous, envelope) {
  ...
  demand(typeof b.action === 'string' && Object.hasOwn(ACTION_FIELDS, b.action), 'UNKNOWN_ACTION'); fields(b.args, ACTION_FIELDS[b.action]);
  ...
  switch (b.action) {
    ...
    case 'deploy': {
      ...
      demand(asset.deployed === null && asset.locked === null, 'ASSET_BUSY');
    ...
    case 'undeploy': { ownerAsset(s, p, a.asset); undock(s, a.asset); break; }
    case 'offer': {
      unique(s.offers, a.id); const asset = ownerAsset(s, p, a.asset);
      demand(asset.deployed === null && asset.locked === null, 'ASSET_BUSY'); money(a.price); integer(a.until, s.tick + 1, s.tick + 1_000);
...
export function assertInvariants(s) {
  ...
  for (const raw of ['ore', 'wood']) {
    ...
    for (const job of Object.values(s.jobs)) if (job.status === 'running') amounts.push(job.inputs[raw] + (job.raw === raw ? 1 : 0));
